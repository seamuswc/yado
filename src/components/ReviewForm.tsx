    ...
    <details className="rounded-xl border border-line bg-paper">
      ...
      <form action={action} className="p-3 space-y-2 border-t border-line">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <select id={`rating-${bookingId}`} name="rating" defaultValue="5" className="rounded-lg border border-line bg-card px-2 py-2 min-h-11 text-base">
            {[5, 4, 3, 2, 1].map((n) => <option key={n} value={n}>{"★".repeat(n)}{"☆".repeat(5 - n)}</option>)}
          </select>
        </div>
        <input name="authorName" defaultValue={defaultName} required maxLength={40} placeholder={dict.reviews.displayName} aria-label={dict.reviews.displayName} className={field} />
        <input name="title" required maxLength={100} placeholder={dict.reviews.reviewTitle} aria-label={dict.reviews.reviewTitle} className={field} />
        <textarea name="body" required minLength={10} maxLength={2000} rows={3} placeholder={dict.reviews.reviewBody} aria-label={dict.reviews.reviewBody} className={field} />
