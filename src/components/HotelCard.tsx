...
}: { hotel: Hotel; locale: Locale; dict: Dictionary; query?: string }) {
  const city = getCity(hotel.city);
  const href = `/${locale}/hotels/${hotel.id}${query ? `?${query}` : ""}`;
  const size = maxSize(hotel);
  return (
    <Link href={href} className="block rounded-2xl bg-card border border-line overflow-hidden shadow-sm active:scale-[0.99] transition">
      <div className="relative aspect-[16/10] bg-line">
        {hotel.images[0] && <Image src={hotel.images[0]} alt={t(hotel.name, locale)} fill sizes="(max-width: 448px) 100vw, 448px" className="object-cover" {...imgOpts(hotel.images[0])} />}
      ...
      <div className="p-3">
        ...
        <p className="mt-2 text-sm flex items-baseline justify-between">
          <span>
